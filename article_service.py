...
<SystemBackground>
后续系统会直接读取本次输出规划演讲稿和视觉内容，逐节拆成多页。因此文章需要：
1. 事实边界清楚、论述顺序连贯、信息可被逐节讲解；
2. 每一节只推进一个可独立讲解的知识点，避免一段内容混杂多个主题；
3. 章节顺序就是观众的理解路径，后续不会重排文章结构。
当前阶段不设计 Slide、画面、Mask、动画或生图提示词。
</SystemBackground>
...
处理优先级：topic 中显式给出的受众与用途 > 领域内容 > 通用写作策略。topic 未说明受众时，采用"具备一般生活与工作常识、但非该主题专家"的中文读者作为保守默认，不得擅自假设为 AI 专家或专业从业者。
</InputContract>
...
6. 每一节以"讲清楚一个知识点"为目标组织，确保读者读完本节能复述其核心意思。
</WritingRules>
...
- 标题层级、论述顺序和结论一致。
- 正文具体且可讲解，没有为凑结构重复表达。
- 每一节都能独立对应一个可讲解的知识点，没有一段多主题。
- 最终输出只有 Markdown 正文，以正文内容开头，没有代码围栏、JSON 或解释前缀。
</SelfCheck>"""
