...
def settings_override_summary(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.layout is not None and not fine_grained:
        return {"source": "--layout", "override": {"doclayout_enabled": args.layout == "on"}}
    if fine_grained and args.layout is None:
        return {"source": "--fine-grained", "override": {"fine_grained_detection": True}}
    override: dict[str, Any] = {}
    if args.layout is not None:
        override["doclayout_enabled"] = args.layout == "on"
    if fine_grained:
        override["fine_grained_detection"] = True
    if override:
        return {"source": "cli", "override": override}
...
def _read_configured_vision() -> dict[str, str]:
    """Capture only the current vision connection before benchmark isolation.

    The isolated benchmark has its own settings store by design.  This opt-in
    bridge copies the active connection into that private store without ever
    printing its key or endpoint.
    """
    from config_store import get_setting

    return {
        "llm_api_key": str(get_setting("llm_api_key", "") or ""),
        "llm_base_url": str(get_setting("llm_base_url", "") or ""),
        "vision_model": str(get_setting("vision_model", "") or get_setting("llm_model", "") or ""),
    }


def _configure_vision_settings(
    server_module: Any,
    args: argparse.Namespace,
    configured_vision: dict[str, str] | None = None,
) -> dict[str, Any]:
    ...
    if configured_vision:
        updates.setdefault("llm_api_key", configured_vision["llm_api_key"])
        updates.setdefault("llm_base_url", configured_vision["llm_base_url"])
        updates.setdefault("vision_model", configured_vision["vision_model"])
        updates.setdefault("llm_model", configured_vision["vision_model"])
    if updates:
        update_settings(updates)
    return {
        "remote_vision_configured": bool(updates.get("llm_api_key") and updates.get("llm_base_url") and updates.get("vision_model")),
        "vision_model": str(updates.get("vision_model") or ""),
...
def _run_case(
    ...
    try:
        ...
        elif args.layout is not None or bool(getattr(args, "fine_grained", False)):
            settings_override = {}
            if args.layout is not None:
                settings_override["doclayout_enabled"] = args.layout == "on"
            if bool(getattr(args, "fine_grained", False)):
                settings_override["fine_grained_detection"] = True
...
def run(args: argparse.Namespace) -> int:
    ...
    if problems:
        ...
        return 2
    configured_vision = _read_configured_vision() if bool(getattr(args, "use_configured_vision", False)) else None
    ...
    vision = _configure_vision_settings(server_module, args, configured_vision)
    ...
    if vision["remote_vision_configured"]:
        max_requests_per_page = 4
        estimate = len(selected) * args.repeats * max_requests_per_page
        print(
            f"[budget] remote vision: {len(selected)} pages x {args.repeats} repeats "
            f"x <= {max_requests_per_page} batches = <= {estimate} AI Mask requests "
            "(each request carries 1 overview + <=12 crops)"
        )
...
def main() -> int:
    ...
    parser.add_argument("--fine-grained", action="store_true", help="enable the fine-grained detector for an A/B arm")
    ...
    parser.add_argument("--use-configured-vision", action="store_true", help="copy the active local vision connection into the isolated benchmark without logging credentials")
