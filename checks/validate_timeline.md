...
- `animation_timeline.events[].action` 必须属于允许动作集合：`fade_in`、`fade_up`、`soft_zoom_in`、`slide_in_left`、`line_draw`、`highlight`、`count_up`。
...
- 重要画面元素要在旁白提到附近出现，不能提前太多。
- 同一时间出现的元素不能过多。
- `main_title` 和 `subtitle` 必须作为两个独立 target 处理。
- 字幕区元素不得成为动画 target。
- 如果精确绑定困难，允许降级为标题、主体、总结三段式动画，但仍需通过 target 和时间校验。
