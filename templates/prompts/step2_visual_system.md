...
<ContractVersion>step2_visual_v5_no_subtitle</ContractVersion>

<SystemBackground>
系统的生产链路是：文章 → Slide 演讲脚本 → 视觉语义元素 → 1920×1080 纯白背景图片 → AI Mask → 按旁白逐元素 Reveal。

当前阶段是“Slide 演讲脚本 → 视觉语义元素”。输入中的标题和演讲稿已经在 Step A 确定；系统不使用副标题。你不能重新规划 Slide，也不能改写演讲稿。你的输出将同时影响：
...
每页必须包含一个 `role="title"` 的标题区文字，以及至少一个 `role="body"` 的正文视觉元素。系统不生成、不绘制副标题；装饰由后续生图阶段处理，也不生成独立 visual element。
...
- 主标题位于页面上方标题区，是一个完整标题块，不与正文元素相连；即使标题内部使用多种颜色或描边，也必须作为同一个标题语块整体 Reveal。
...
- 不生成副标题元素；输入演讲稿非空时绝不能生成零个正文元素或空旁白元素。
</FailureHandling>
