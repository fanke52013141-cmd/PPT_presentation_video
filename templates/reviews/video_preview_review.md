...
## 检查项（括号内为该检查项失败时建议的状态）

- 语音是否自然，是否有明显机械感、怪异停顿或不合适语气标签？（→ revise_audio）
- 字幕是否准确，是否和语音内容一致？（→ revise_audio）
- 字幕是否保持单行显示？（→ revise_audio）
- 字幕是否居中靠下，且不遮挡开放内容区主体信息？（→ revise_slide 或 revise_animation）
- 字幕中是否没有出现 TTS 控制标签，例如停顿标记或语气标签？（→ revise_audio）
- 动画是否跟随旁白，不提前暴露重点信息？（→ revise_animation）
- 页面节奏是否过快或过慢？（→ revise_audio 或 revise_animation）
- 是否有黑屏、静音、花屏、资源缺失或图片丢失？（→ revise_scene）
- 是否有音画不同步、字幕不同步或动画不同步？（→ 按最重问题取对应状态）
- 标题、正文是否清晰可读？（→ revise_slide）
- 底部字幕安全区内是否只出现视频字幕，没有 PPT 内容或装饰元素？（→ revise_scene）
- `rough_cut.mp4` 是否已生成并通过审核后，才进入 `final.mp4` 导出？（→ 阻断）
